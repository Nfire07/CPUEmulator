...
public class InstructionSetViewer extends Thread {
    JFrame window;

    public void run() {
        SwingUtilities.invokeLater(() -> {
            window = new JFrame("Instruction Set");
            ...
            String[] columns = { "OPCODE", "INSTRUCTION" };
            ...
            scrollPane.setBackground(EditorASM.editorBackgroundColor);
    ...
    public void loadInstructionSetIntoTable(DefaultTableModel model) {
        ...
        for (Map.Entry<String, String> entry : CPU.instructionSet.entrySet()) {
            tablelist.add(new String[]{entry.getKey(), entry.getValue()});
        }
        tablelist.sort(new Comparator<>() {
            @Override
